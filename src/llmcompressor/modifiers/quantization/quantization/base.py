...
class QuantizationModifier(Modifier, QuantizationMixin):
    ...
    def on_sequential_epoch_end(
        ...
        if not is_distributed():
            ...
            # Only modules with a weight tensor can participate in weight bin
            # packing. Modules that are quantized solely for input-activation
            # (e.g. kv_cache) have no .weight attribute and would crash
            # greedy_bin_packing.
            weight_modules = [
                m for m in modules if getattr(m, "weight", None) is not None
            ]
