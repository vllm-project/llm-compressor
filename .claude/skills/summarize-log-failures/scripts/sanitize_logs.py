#!/usr/bin/env python3
"""
Sanitize test logs by removing ANSI codes, timestamps, and other noise.
Usage: python sanitize_logs.py <input_dir> [output_dir] [--aggressive]
"""
...
def remove_ci_metadata(text: str) -> str:
    """Remove CI-specific metadata and decorative elements."""
    # Remove lines with box drawing characters
    text = re.sub(r'^[│║╔╗╚╝═─┌┐└┘├┤┬┴┼]+\s*$', '', text, flags=re.MULTILINE)
    # Remove lines that are purely decorative borders
    text = re.sub(r'^[=*-]{3,}\s*$', '', text, flags=re.MULTILINE)
    # Remove "~~~ " prefix that some CI systems add
...
def sanitize_log(log_content: str, aggressive: bool = False) -> str:
    ...
    sanitized = remove_ci_metadata(sanitized)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python sanitize_logs.py <input_dir> [output_dir] [--aggressive]")
        sys.exit(1)
