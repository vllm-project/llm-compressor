...
# In this case, we:
#   * apply SmoothQuant to migrate quantization difficulty from activations to
#       weights via channel-wise scaling before quantization
#   * quantize the weights to fp4 with per group 16 via GPTQ
...
# Apply quantization using the prebaked "ultrachat_200k" calibration dataset.
...
SAVE_DIR = MODEL_ID.rstrip("/").split("/")[-1] + "-NVFP4-GPTQ-SmoothQuant-ultrachat"
