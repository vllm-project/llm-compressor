"""Perplexity evaluation for NVFP4 observer comparison.

Evaluates unquantized baselines and compressed models on Wikitext-2,
outputting results in RESULTS.md table format.

Uses sliding-window perplexity (max_length=2048, stride=512).

Usage:
    python eval_all.py
    python eval_all.py --models meta-llama/Meta-Llama-3-8B
    python eval_all.py --configs fouroversix mse-1x-1.5x
    python eval_all.py --no-baseline  # skip unquantized baseline
"""
...
def _max_memory():
    """Reserve 10 GiB per visible GPU for decompression overhead, with CPU fallback."""
    n = torch.cuda.device_count()
    if n == 0:
        return None
    per_gpu = torch.cuda.get_device_properties(0).total_memory
    limit = int((per_gpu - 10 * 2**30))
    mem = {i: limit for i in range(n)}
    mem["cpu"] = "100GiB"
    return mem
...
def load_and_eval(path: str, label: str) -> float:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        path, device_map="auto", torch_dtype="auto", max_memory=_max_memory()
    )
    model.eval()
