...
def test_sequential_activation_qparams_only_updated_once_per_module():
    ...
    model = nn.Sequential(nn.Linear(10, 10), nn.Linear(10, 10), nn.Linear(10, 10))
    act_args = QuantizationArgs(num_bits=8, type="int", symmetric=True, strategy="tensor")
    for layer in model:
        initialize_module_for_quantization(
    ...
    with patch.object(
        model[0].input_observer, "get_qparams", wraps=model[0].input_observer.get_qparams
    ) as qp_mock0, patch.object(
        model[1].input_observer, "get_qparams", wraps=model[1].input_observer.get_qparams
    ) as qp_mock1, patch.object(
        model[2].input_observer, "get_qparams", wraps=model[2].input_observer.get_qparams
        ...
        modifier.on_event(state, Event(type_=EventType.SEQUENTIAL_EPOCH_END), modules=[model[1]])
        modifier.on_event(state, Event(type_=EventType.SEQUENTIAL_EPOCH_END), modules=[model[2]])
