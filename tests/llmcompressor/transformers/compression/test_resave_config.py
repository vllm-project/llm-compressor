...
class FakeConfig(PretrainedConfig):
    """
    Thin alias for ``PretrainedConfig``. ``resave_config`` sources patched field
    values from ``config.to_dict()`` and locates the original config via
    ``_name_or_path``, both of which the base class provides directly.
    """
...
def save_dir(tmp_path):
    ...
    _write_json(
        ...
        {"dtype": "float32", "serialized_by": "transformers"},
    )
    return save_dir
...
def test_missing_name_or_path_keeps_serialized_config(save_dir):
    ...
    config = FakeConfig(name_or_path="", dtype="float32")

    resave_config(config, str(save_dir))

    assert _read_json(save_dir / "config.json") == {
        "dtype": "float32",
        "serialized_by": "transformers",
    }


def test_resave_original_config_without_experts(save_dir, original_dir):
    original = {"architectures": ["Foo"], "hidden_size": 4, "torch_dtype": "float32"}
    ...
    # (dtype is patched to the same value it already held)
...
def test_resave_config_is_sorted_and_indented(save_dir, original_dir):
    ...
    assert contents == json.dumps(
        {"a": 2, "b": 1, "torch_dtype": "float32"}, indent=2, sort_keys=True
    )
...
def test_experts_field_patched_top_level(save_dir, original_dir):
    ...
    _write_json(
        ...
        {"num_experts": 8, "hidden_size": 4, "torch_dtype": "float32"},
    )
    config = FakeConfig(name_or_path=str(original_dir), dtype="float32", num_experts=4)
...
def test_experts_field_patched_via_key_fallback(save_dir, original_dir):
    ...
    _write_json(
        original_dir / "config.json",
        {"num_local_experts": 8, "torch_dtype": "float32"},
    )
    config = FakeConfig(name_or_path=str(original_dir), dtype="float32", num_experts=4)
...
def test_experts_field_patched_in_nested_text_config(save_dir, original_dir):
    _write_json(
        original_dir / "config.json",
        {
            "text_config": {"num_experts": 8, "torch_dtype": "float32"},
            "model_type": "multimodal",
        },
    )
    text_config = FakeConfig(num_experts=4, dtype="float32")
    config = FakeConfig(
        name_or_path=str(original_dir), dtype="float32", text_config=text_config
    )
...
def test_tie_word_embeddings_patched(save_dir, original_dir):
    ...
    config = FakeConfig(
        name_or_path=str(original_dir), dtype="float32", tie_word_embeddings=False
    )
...
def test_dtype_missing_target_key_keeps_serialized(save_dir, original_dir):
    # dtype is required: the original has no dtype key to patch, so the resave is
    # atomic and the transformers-serialized config is kept
...
def test_experts_present_but_missing_target_key_keeps_serialized(
    ...
    _write_json(
        original_dir / "config.json", {"hidden_size": 4, "torch_dtype": "float32"}
    )
    config = FakeConfig(name_or_path=str(original_dir), dtype="float32", num_experts=4)
...
def test_partial_failure_keeps_serialized_config(save_dir, original_dir):
    ...
    _write_json(
        original_dir / "config.json", {"num_experts": 8, "torch_dtype": "float32"}
    )
    config = FakeConfig(
        name_or_path=str(original_dir),
        dtype="float32",
        num_experts=4,
...
def test_falls_back_to_hf_hub_download(save_dir, original_dir, tmp_path, monkeypatch):
    ...
    _write_json(
        downloaded,
        {"downloaded": True, "num_experts": 8, "torch_dtype": "float32"},
    )
    ...
    config = FakeConfig(name_or_path="some/remote-repo", dtype="float32", num_experts=4)
