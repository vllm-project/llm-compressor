...
@requires_gpu
@pytest.mark.parametrize("scheme", ["FP8_dynamic"])
...
def _assert_safetensors_equal(a_path: str, b_path: str) -> bool:
    ...
    for key in a.keys():
        ...
        value_close = torch.allclose(
            a[key].to(torch.bfloat16), b[key].to(torch.bfloat16)
        )
        dtype_equal = a[key].dtype == b[key].dtype

        assert value_equal and dtype_equal, (
            key,
            value_equal,
            value_close,
            dtype_equal,
        ...
        if not (value_equal and dtype_equal):
            breakpoint()
