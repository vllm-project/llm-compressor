...
def test_calib_qwen3_5_moe_module():
    ...
    with torch.device("cuda"):
        original = Qwen3_5MoeSparseMoeBlock(text_config).eval()
        # Fused expert tensors and router weight default to empty/zeros;
        # initialize with small random values for meaningful outputs
        original.experts.gate_up_proj.data.normal_(mean=0.0, std=0.02)
        original.experts.down_proj.data.normal_(mean=0.0, std=0.02)
        original.gate.weight.data.normal_(mean=0.0, std=0.02)
