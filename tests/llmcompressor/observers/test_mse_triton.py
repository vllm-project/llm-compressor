...
def _reference_search(observed, args, patience=NO_PATIENCE):
    ...
    for step in range(total_steps):
        ...
        stalled = torch.where(
            active, torch.where(improved, 0, stalled + 1), stalled
        )
...
def _returned_objective(observed, chosen, errors):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            4, "float", True, torch.float8_e4m3fn, id="fp4-e2m1"
        ),
    ],
)
def test_matches_normalized_reference(
    dtype, num_bits, qtype, symmetric, scale_dtype
):
    torch.manual_seed(0)
...
def test_range_survives_the_real_objective(
    ...
    args = _args(
        num_bits, qtype, symmetric=symmetric, scale_dtype=scale_dtype
    )

    assert can_use_triton(observed, args)
    got = grid_search_triton(
        observed, args, MAXSHRINK, patience, GRID, NORM
    )
...
def test_patience_actually_fires():
    ...
    full = grid_search_triton(
        observed, args, MAXSHRINK, NO_PATIENCE, GRID, NORM
    )
    early = grid_search_triton(observed, args, MAXSHRINK, 1, GRID, NORM)
    assert not torch.equal(full[0], early[0]), (
...
def test_zero_patience_means_one_not_none():
    ...
    assert not torch.equal(got[0], unshrunk), (
        "patience=0 searched nothing"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            torch.zeros(1, 8, 4, 32),
            QuantizationArgs(
                num_bits=8, type="float", symmetric=False,
                strategy=QuantizationStrategy.GROUP, group_size=32,
            ),
...
def test_codebooks_cover_symmetric_asymmetric_and_fp4():
    ...
    fp4 = _codebook(
        _args(4, "float", scale_dtype=torch.float8_e4m3fn)
    )
...
def test_bound_covers_the_jump_the_data_makes(
    num_bits, qtype, symmetric, scale_dtype
):
    ...
    for zp in zps:
        u = torch.linspace(
            float(codes[0]) - zp - 1, float(codes[-1]) - zp + 1, 8192
        )
        prev = None
...
def test_grid_search_mse_actually_dispatches(monkeypatch):
    ...
    assert calls[0][3] == 5, "patience was not forwarded to the kernel"
    want = _reference_search(observed, args, patience=5)
    assert torch.equal(got[0], want[0])
    assert torch.equal(got[1], want[1])
...
def test_other_dispatched_layouts_match_reference(strategy_args, symmetric):
    ...
    assert can_use_triton(observed, args), (
        f"{strategy_args} no longer dispatches; the claim below would "
        "be vacuous"
    )
    got = grid_search_triton(
        observed, args, MAXSHRINK, NO_PATIENCE, GRID, NORM
