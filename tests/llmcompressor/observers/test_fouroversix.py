...
def test_fouroversix_searches_only_1x_and_1_5x():
    ...
    orig_eager = (
        __import__(
            "llmcompressor.observers.mse_quant", fromlist=["_grid_search_eager"]
        )
    )._grid_search_eager
    ...
    with patch(
        "llmcompressor.observers.mse_quant._grid_search_eager", patched_eager
    ):
        observer(weight)

    assert len(shrink_factors) == 2, f"Expected 2 search points, got {len(shrink_factors)}"
    assert shrink_factors[0] == pytest.approx(1.5, abs=1e-4), (
        f"First search point should be 1.5x, got {shrink_factors[0]}"
    )
    assert shrink_factors[1] == pytest.approx(1.0, abs=1e-4), (
